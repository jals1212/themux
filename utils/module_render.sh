#!/usr/bin/env bash
# Assemble @themux_module_<name> from the v3 props (shape/leading/text/notch),
# mirroring utils/pane_render.sh: the leading (icon) and text blocks are styled
...
"#{?#{@themux_module_shape},#{@themux_module_shape},#{@themux_all_shape}}${US}#{?#{@themux_module_leading},#{@themux_module_leading},#{@themux_all_leading}}${US}#{?#{@themux_module_text},#{@themux_module_text},#{@themux_all_text}}${US}#{?#{@themux_module_notch},#{@themux_module_notch},#{@themux_all_notch}}${US}#{@themux_module_connect_separator}${US}#{@themux_${name}_color}${US}#{@thm_surface_0}${US}#{@thm_crust}${US}#{@thm_fg}${US}#{@themux_${name}_icon}${US}#{@themux_${name}_icon_bg}${US}#{@themux_${name}_icon_fg}${US}#{@themux_${name}_text_bg}${US}#{@themux_${name}_text_fg}${US}#{@themux_${name}_self_styled}${US}#{@themux_module_middle_separator}${US}#{@themux_${name}_when}${US}#{?#{@themux_module_leading_highlight},#{@themux_module_leading_highlight},#{@themux_all_leading_highlight}}${US}#{?#{@themux_module_text_highlight},#{@themux_module_text_highlight},#{@themux_all_text_highlight}}${US}#{?#{@themux_module_leading_position},#{@themux_module_leading_position},#{@themux_all_leading_position}}${US}#{E:@themux_${name}_color}${US}END")
...
midsep=${V[15]} when=${V[16]} lead_hl=${V[17]} text_hl=${V[18]} position=${V[19]}
accent_exp=${V[20]}
[ -n "$lead_hl" ] || lead_hl=both
...
resolve_style "$leading" "$accent" "$surface" "$crust" "$fg"
ibg="$RS_BG" ifg="$RS_FG"
case "$lead_hl" in bg|both) [ -n "$ibg_ov" ] && ibg="$ibg_ov" ;; esac
case "$lead_hl" in fg|both) [ -n "$ifg_ov" ] && ifg="$ifg_ov" ;; esac
